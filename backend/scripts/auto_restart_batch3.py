#!/usr/bin/env python3
"""
Auto-restart Batch 3 enrichment process.
...
def get_batch3_status(conn):
    """Get Batch 3 status"""
    with conn.get_session() as session:
        total_result = session.run("""
            MATCH (w:Word)
            WHERE w.frequency_rank >= 2001 AND w.frequency_rank <= 3000
...
def is_process_running():
    """Check if Batch 3 enrichment process is running"""
    try:
        result = subprocess.run(
            ["pgrep", "-f", "agent_batched.*2001.*3000"],
            capture_output=True,
            text=True
        )
...
def restart_process():
    """Restart Batch 3 enrichment process"""
    print(f"\n{'='*70}")
    print(f"🔄 Restarting Batch 3 enrichment - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
...
def main():
    ...
    print("=" * 70)
    print("🤖 Batch 3 Auto-Restart Monitor")
    print("=" * 70)
    ...
    try:
        while True:
            ...
            try:
                ...
                if pending == 0:
                    print("\n" + "=" * 70)
                    print("🎉 Batch 3 Complete! All words enriched.")
                    print("=" * 70)
                    break
